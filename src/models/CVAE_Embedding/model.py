...
model.py
================
活動スケジュール生成のためのConditional VAE
追加要素(Embedding層, Early Stopping)

設計方針:
    - スケジュール表現  : 96スロット × 10状態の固定グリッドを one-hot 平坦化（960次元）
    - 条件表現: 5属性を one-hot 連結 → Embedding
...
DATA_PATH   = REPO_ROOT / 'data' / 'processed' / 'weighted_dataset.csv'
MODEL_SAVE_PATH = REPO_ROOT / 'outputs' / 'checkpoints' / 'cvae_embedding_non_stopping.pt'
...
NUM_WORKER  = 2
COND_DIM    = AGE_BINS + NUM_GENDER + NUM_MEMBER + NUM_ROLE + NUM_WORKER  # = 33（参考: 旧 one-hot 次元）

# 条件の埋め込み（one-hot を学習可能 Embedding に置換）
# 並びは load_data の cond_idx と一致: [age, gender, member, role, worker]
COND_CARD    = [AGE_BINS, NUM_GENDER, NUM_MEMBER, NUM_ROLE, NUM_WORKER]  # 各属性のカテゴリ数
EMB_DIMS     = [4, 2, 4, 4, 2]                                            # 各属性の埋め込み次元
COND_EMB_DIM = sum(EMB_DIMS)                                              # = 16（条件ベクトルの次元）
...
# Early stopping（val loss を監視）
EARLY_STOPPING = False # Early stoppingをするかどうか
PATIENCE    = 20      # val が改善しないエポックがこの数に達したら打ち切り
MIN_DELTA   = 1e-3    # 「改善」とみなす val loss の最小減少量
...
# 2. データ整形
# ============================================================
def load_data(path: str | Path) -> tuple[npt.NDArray[np.int64], npt.NDArray[np.int64], npt.NDArray[np.float64]]:
...
# ============================================================
# 3. モデル（CVAE - Embedding）
# ============================================================
class CVAE(nn.Module):
    def __init__(self):
        super().__init__()
        # 条件の埋め込み: 5属性それぞれを Embedding し連結（one-hot の置換）
        self.cond_embeds = nn.ModuleList([
            nn.Embedding(card, dim) for card, dim in zip(COND_CARD, EMB_DIMS)
        ])

        # encoder: [x_onehot(960) + cond_emb(16)] -> hidden -> (mu, logvar)
        ...
        # decoder: [z(64) + cond_emb(16)] -> hidden -> 96*10 ロジット
    ...
    # 条件インデックス (B,5) -> 埋め込み連結 (B, COND_EMB_DIM)
    def embed_cond(self, cond_idx):
        return torch.cat([emb(cond_idx[:, i]) for i, emb in enumerate(self.cond_embeds)], dim=1)

    # エンコーダー
    def encode(self, x_onehot, cond):
        h = self.encoder(torch.cat([x_onehot, cond], dim=1))  # [x_onehot(960) + cond_emb(16)] -> [h(512)]
        return self.fc_mu(h), self.fc_logvar(h)  # [h(512)] -> [μ(64)], [h(512)] -> [logσ^2(64)]
    ...
    # デコーダー（cond は埋め込み済みベクトル）
    def decode(self, z, cond):
        logits = self.decoder(torch.cat([z, cond], dim=1))  # [z(64) + cond_emb(16)] -> [logits(96*10)]
        return logits.view(-1, NUM_SLOTS, NUM_ACT)   # (B, 96, 10)

    def forward(self, sched, cond_idx):
        cond = self.embed_cond(cond_idx)  # (B,5) -> (B, COND_EMB_DIM)
        x_onehot = F.one_hot(sched, NUM_ACT).float().view(sched.size(0), -1)
        mu, logvar = self.encode(x_onehot, cond)  # encoder input: [act, cond_emb]
        z = self.reparameterize(mu, logvar)
        logits = self.decode(z, cond)  # decoder input: [z, cond_emb]
...
def run_epoch(model, loader, optimizer=None):
    ...
    with torch.set_grad_enabled(is_train):
        for cond_idx, sched in loader:
            ...
            logits, mu, logvar = model(sched, cond_idx)  # 条件は内部で Embedding
...
def train():
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        # Early stopping: val loss が MIN_DELTA 以上改善したらベスト更新、なければカウント
    ...
    # ベスト重みを復元して保存（最終エポックではなく val 最良を採用）
    model.load_state_dict(best_state)
    MODEL_SAVE_PATH.parent.mkdir(parents=True, exist_ok=True)
    torch.save(model.state_dict(), MODEL_SAVE_PATH)
    print(f"saved best model (epoch {best_epoch}, val {best_val:.3f}) to {MODEL_SAVE_PATH}")
...
def generate(model, cond_idx, sample=True):
    ...
    cond = model.embed_cond(ci)                    # (M,5) -> (M, COND_EMB_DIM)
